...
\begin{definition}
  We choose an arbitrary countably infinite set $A$ of \emph{propositional
  atoms}, and we define \emph{propositions} as follows.
  \begin{enumerate}[1.]
    \item Each propositional atom is a proposition.
    \item If $\alpha$ is a proposition, then $\neg\alpha$ is a proposition.
    \item If $\alpha$ and $\beta$ are propositions, then $(\alpha \to \beta)$
    is a proposition.
  \end{enumerate}
\end{definition}

\begin{definition}
  A \emph{truth assignment} is a map $\tau: A \to \{0, 1\}$, and we further
  define
  \begin{equation*}
    \tau(\neg\alpha) =
    \begin{cases}
    ...
    \end{cases}
  \end{equation*}
  for each formula $\alpha$ and $\beta$.
  We say that $\tau$ \emph{satisfies} formula $\alpha$ if $\tau(\alpha) = 1$.
\end{definition}
