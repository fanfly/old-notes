...
\begin{theorem}
  ...
  $T \in \mathcal{L}(V)$.
  If $\lambda$ is an eigenvalue of $T$, then $\overline{\lambda}$ is an
  eigenvalue of $T^*$.
\end{theorem}
\begin{proof}
  To prove $\overline{\lambda}$ is an eigenvalue of $T^*$, it suffices to prove
  that $\mathcal{N}(T^* - \overline{\lambda}I_V)$ consists of an nonzero
  vector, i.e.,
  \begin{equation*}
    \nullity(T^* - \overline{\lambda}I_V) \geq 1.
  \end{equation*}

  Suppose that $(\lambda, x)$ be an eigenpair of $T$.
  Then for any $y \in V$, we have
  \begin{align*}
    \inner{x}{(T^* - \overline{\lambda}I_V)(y)}
    &= \inner{x}{(T - \lambda I_V)^*(y)} \\
    &= \inner{(T - \lambda I_V)(x)}{y} \\
    &= \inner{0_V}{y} \\
    &= 0_F.
  \end{align*}
  Thus, $x \in (\mathcal{R}(T^* - \overline{\lambda} I_V))^\perp \setminus
  \{0_V\}$.
  (To be completed.)
\end{proof}
